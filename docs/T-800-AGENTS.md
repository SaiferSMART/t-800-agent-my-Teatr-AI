...
Плагин: **T-800 Agent** (текущая версия — `.cursor-plugin/plugin.json`) — `/t800-loop` (Loop Engineering v2) + `/t800-fix` + `/t800-doctor` + `t800_run_gate.py --strict-create` + factory-bypass gate + Teya adapter (`adapters/teya/`) + fix-pack из audit + `/t800-plugin-audit` + DEEP research + factory + `/t800-cloud-hub`.
